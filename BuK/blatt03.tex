...
\textit{L} = \{$\langle M \rangle w$ $|$ \textit{M} schreibt bei Eingabe \textit{w} mindestens einmal ein \# aufs Band\}\\

Eingabe: \textit{w}, TM \textit{M}, $|Q|\cdot |\Gamma | \cdot max\{ |w|,1 \}$\\

"Ahnlich wie der Beweis von a) , pr"ufe, ob ein \# in der erreichten Konfiguration geschrieben wird.

\subsection*{c)}

\textit{L} = \{$\langle M \rangle$ $|$ \textit{M} schreibt bei dem leeren Eingabe mindestens einmal ein $a\in \Gamma \ mit\ a\notin B$ aufs Band\}\\
